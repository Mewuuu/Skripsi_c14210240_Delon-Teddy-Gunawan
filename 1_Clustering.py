...
if "df" in st.session_state:
    ...
    if method == "K-Means" and features == "LRFM":
        # --- Compute LRFM ---
        st.subheader("Computing LRFM features")
        ...
        lrfm_scaled = mapped_scores * 0.25
        ...
        # Allow user to set maximum k for evaluation
        ...
        pca_result = pca.fit_transform(lrfm_scaled)
        lrfm_df['PCA1'] = pca_result[:, 0]
        lrfm_df['PCA2'] = pca_result[:, 1]

        st.subheader("Customer Segmentation (2D Projection)")
        plt.figure(figsize=(10, 6))
        sns.scatterplot(data=lrfm_df, x="PCA1", y="PCA2", hue="Cluster", palette="tab10")
    ...
    # --- Kmeans MLRFM ---
        ...
        st.subheader("Customer Segmentation (2D Projection)")
        plt.figure(figsize=(10, 6))
        sns.scatterplot(data=lrfm_df, x="PCA1", y="PCA2", hue="Cluster", palette="tab10", legend="full")
        plt.title("DBSCAN Cluster Visualization with PCA")
        st.pyplot(plt.gcf())
    ...
    produk_per_transaksi = (
        filtered_df.groupby(['customer_id', 'no_invoice'])['qty'].sum().reset_index(name='total_qty')
    )
    avg_qty_per_customer = (
        produk_per_transaksi.groupby('customer_id')['total_qty'].mean().mean()
    )
    ...
    # Tampilkan hasil
    st.markdown(f"**💰 Rata-rata jumlah uang per transaksi per customer:** Rp {avg_uang_per_customer:,.0f}")
    st.markdown(f"**📦 Rata-rata jumlah produk per transaksi per customer:** {avg_qty_per_customer:.2f} item")
    st.markdown(f"**⏱️ Rata-rata rentang waktu antar transaksi per customer:** {avg_rentang_per_customer:.2f} hari") 

    st.subheader("📆 Jumlah Transaksi per Tahun")
    tahun_df = (
    ...
    st.subheader("📅 Jumlah Transaksi per Bulan (Total Semua Tahun)")
    ...
    st.subheader("📅 Jumlah Transaksi per Hari (Senin–Minggu)")
    hari_df = (
